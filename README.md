...
- **Natural Language Processing:** Races Google Gemini (`gemini-3.1-flash-lite`) and Groq (`llama-3.1-8b-instant`) APIs concurrently to provide the fastest possible response with a 5-second timeout fallback. When the backend receives raw words, the winner constructs a natural English sentence.
...
```
Make sure you have your Gemini and Groq API keys ready. You can either export them as environment variables (`GEMINI_API_KEY`, `GROQ_API_KEY`) or save them in `api_key.txt` and `api_key_groq.txt` in the root folder.
